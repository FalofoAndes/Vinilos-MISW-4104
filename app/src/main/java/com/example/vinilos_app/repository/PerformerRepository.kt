...
class PerformerRepository(private val context: Context) {
    ...
    fun getPerformerDetail(musicianId: Int, onComplete: (Performer?) -> Unit, onError: (VolleyError) -> Unit) {
        networkService.getPerformerDetail(musicianId, onComplete, onError)
    }
}